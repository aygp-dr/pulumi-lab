...
*Important:* Pulumi doesn't officially support FreeBSD and is not available in ports/pkg. This project runs Pulumi 3.145.0 via FreeBSD's Linux compatibility layer. Docker daemon doesn't run natively on FreeBSD, so we use Minio as a LocalStack alternative for S3 emulation. See [[file:docs/FREEBSD.org][FreeBSD Guide]] for complete setup.
...
#!/bin/bash
# Test that everything is working locally

echo "=== Testing Pulumi Setup ==="
pulumi version || echo "Pulumi not installed - run: gmake freebsd-setup"
...
if pgrep minio >/dev/null; then
    ...
    echo "Minio not running - run: gmake minio-start"
fi

echo ""
echo "=== Experiment Count ==="
echo "Found $(ls -d experiments/*/ | wc -l) experiments"
#+end_src

** Run Minio Experiment

#+begin_src bash :dir . :results output
#!/bin/bash
# Quick test with Minio S3 experiment
